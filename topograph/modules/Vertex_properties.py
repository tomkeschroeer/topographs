...
class Matcher:
    def __init__(
        ...
        self.phi_jet = list(calc_phi_jet_array(self.phi_truth, self.dR_truth, self.eta_truth, self.eta_jet))
        self.phi_reco = list(calc_phi_reco_array(self.phi_jet, self.dR_reco, self.eta_reco, self.eta_jet))
    ...
    def matching_truth_tracks(self):
        ...
        """
        DeltaR_matrix = ma.masked_equal(calc_delta_R_squared_matrix(self.truth_infos, self.reco_infos), np.nan)
        print(DeltaR_matrix)
