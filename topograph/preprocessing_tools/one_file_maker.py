                ...
                for step in range(n_steps):
                    ...
                    if step == 0 and input_file_ind == 0:
                        ...
                        with File(f"{self.config.output}/{self.config.one_file_name}", "a") as out_file:
                            ...
                            out_file[self.config.input_tracks_name][-n_entries:] = f[f"/{self.config.input_tracks_name}"][step*stepsize:(step+1)*stepsize]
                            out_file[self.config.input_truth_name].resize((out_file[self.config.input_truth_name].shape[0] + n_entries), axis=0)
                            out_file[self.config.input_truth_name][-n_entries:] = f[f"/{self.config.input_truth_name}"][step*stepsize:(step+1)*stepsize]
                            out_file[self.config.input_jet_name].resize((out_file[self.config.input_jet_name].shape[0] + n_entries), axis=0)
                            out_file[self.config.input_jet_name][-n_entries:] = f[f"/{self.config.input_jet_name}"]["HadronConeExclExtendedTruthLabelID"][step*stepsize:(step+1)*stepsize]
