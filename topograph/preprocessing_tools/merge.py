...
from topograph.modules.tools import get_logger
...
class Merge:
    ...
    def Run(self):
        logger = get_logger()
        with File(
            f"{self.config.output}/training_files/training_dataset_merged.h5", "w"
        ) as h5fw:
            row1 = {}
            logger.info("Merging datasets...")
            for i, h5name in enumerate(
                glob(f"{self.config.output}/training_files/*.h5")
            ):
                if "merged" in h5name:
                    continue
                ...
                with File(h5name, "r") as h5fr:
                    for key in list(h5fr.keys()):
                        ...
                        if i == 0:
                            h5fw.create_dataset(
                                key, dtype="f", shape=(shape), maxshape=(maxshape)
                            )
                            row1[key] = dslen
                        else:
