                ...
                if step == 0 and ninput == 0:
                    with File(output_file, "w") as train_file:
                        train_file.create_dataset(
                            self.config.vertex_feat_name,
                            data=datasets.get_vertex_feat_y(),
                            chunks=True,
                            maxshape=(None,),
                        )  # len(global_conf.vertex_features))) # dtype=datasets.vertex_feat_dtypes
                        ...
                        train_file.create_dataset(
                            self.config.tracks_name,
                            data=np.array(
                                datasets.get_track_input(), dtype=datasets.reco_dtypes
                            ),
                            chunks=True,
                            maxshape=(None, 40),
                        )
                        ...
                        train_file.create_dataset(
                            "track_extra_truth",
                            data=datasets.get_extra_track_truth(),
                            chunks=True,
                            maxshape=(
                                None,
                                40,
                        ...
                        train_file.create_dataset(
                            "unscaled_pt",
                            data=datasets.get_unscaled_pt(),
                            chunks=True,
                            maxshape=(None,),
                        )
                        train_file.create_dataset(
                            "jet_pt",
                            data=datasets.get_jet_pt(),
                            chunks=True,
                            maxshape=(None,),
                        )
                        train_file.create_dataset(
                            "truthOriginLabel",
                            data=datasets.get_truthOriginLabel(),
                            chunks=True,
                            maxshape=(None,),
                        )
                else:
