            ...
            for step in range(n_steps):
                ...
                if step == 0 and input_file_ind == 0:
                    with File(f"{training_file_dir}/{self.config.training_file_name}", "w") as train_file:
                        ...
                        train_file.create_dataset(self.config.track_name, data = datasets.get_track_input(), chunks=True, maxshape=(None,40,len(global_conf.track_inputs)))
                else:
                    ...
                    with File(f"{training_file_dir}/{self.config.training_file_name}", "a") as train_file:
                        ...
                        train_file[self.config.track_name].resize((train_file[self.config.track_name].shape[0] + njets_step), axis=0)
                        train_file[self.config.track_name][-njets_step:] = datasets.get_track_input()
