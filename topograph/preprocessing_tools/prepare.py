...
class Prepare:
    ...
    def Run(self):
        ...
        for jet_type in jet_types:
            ...
            output_file_jettype = f"{output_file}_{jet_type}.h5" if len(jet_types) > 1 else f"{output_file}.h5"
            for ninput, input_file in enumerate(updated_filelist):
                ...
                for step in range(n_steps):
                    logger.info(f"Process file {input_file}, step {step+1}/{n_steps} for jet type {jet_type}.")
                    ...
                    if create_file:
                        ...
                        if njets_step > 0:
                            with File(output_file_jettype, "a") as train_file:
                                ...
                                logger.info(
                                    "loaded "
                                    + str(len(train_file[f"{self.config.vertex_feat_name}"]))
                                    + " jets in total"
                                )
                                if len(train_file[f"{self.config.vertex_feat_name}"]) >= int(
                                    njets
                                ):
